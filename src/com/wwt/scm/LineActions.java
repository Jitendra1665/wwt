...
public class LineActions {
	...
	@FindBy(css=" ul[class='nav pull-left lines-dropdown'] li[class='dropdown open'] a[class='dropdown-toggle ng-binding']")
	...
	@FindBy(css="div.nav-collapse ul.nav li.dropdown a.dropdown-toggle.ng-binding")
	private WebElement LineResultsdropdown;
	...
	//@FindBy(css="div.span4 div.actions-dropdown button.btn")
	@FindBy(css="div.nav-collapse ul.nav li.dropdown a[class='dropdown-toggle'][ng-show*='!search']")
	...
	@FindBy(css="ul.dropdown-menu.actions-dropdown-list>li>a")
	...
	@FindBy(css="div#linesTable.ng-scope div.pull-left div.lines-cell a")
	...
	//@FindBy(css="div#linesTable.ng-scope div.pull-left")
	...
	//@FindBy(css="div#linesHeaderContainer.ng-scope div#linesHeader div.result-headers div.ng-binding")
	...
	public boolean searchSites(String searchString) throws InterruptedException{
			siteserachInput.sendKeys(searchString);
			Thread.sleep(6000);
			return true;
	}
	// Method to click on Move button based on country
	public boolean clickMovebasedonCountry(String Country){
		//if(movecountryList.size()==movebuttonlist.size()){
			for(int i=0; i<movebuttonlist.size();i++){
				System.out.println(getDatacellValue("Country",0));
				//if(movecountryList.get(i).getText().equalsIgnoreCase(Country)){
				if(getDatacellValue("Country",i).equalsIgnoreCase(Country)){
	...
	public String getDatacellValue(String ColName, int rownumber){
    	...
    	String desiredColCss= "//div[@id='linesTable']/div["+ colnum +"]/div";
